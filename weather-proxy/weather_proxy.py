...
logger = setup_logger("loggerWeatherProxy")
    ...
    # Dictionary of direct swaps
    # THESE ARE PROCESSED IN ORDER, put longer phrases first!
condition_map = {
        ...
        "Areas Of": " ", # Replacing with empty string to just delete string
        "Partly Sunny": "P. Snny",
        "Light": "Lgt",
        "Chance": "Chc",
        "Slight": "Slgt",
...
def shorten_forecast(description):


    
# 1. Create an empty list to act as our accumulator
#    shortened_words = []
    
    # 2. Split the incoming NWS string into individual words
#    words = description.split()
    
    # 3. Loop through every single word in the phrase
#    for word in words:
        # 4. Check if the word exists in our dictionary
#        if word in condition_map:
            # If it does, add the ABBREVIATED version to our list
#            shortened_words.append(condition_map[word])
#        else:
            # If it DOES NOT, add the ORIGINAL word to our list 
            # (This prevents us from losing words like "And" or "With")
#            shortened_words.append(word)
            
    # 5. Stitch the accumulated list back together into a single string with spaces
#    final_string = " ".join(shortened_words)
    
#    return final_string
    

# Loop through the dictionary and replace any found words
     shortened = description
     for target_word, replacement in condition_map.items():
         shortened = shortened.replace(target_word, replacement)

     return shortened.strip()

def degrees_to_compass(d):
# """Converts meteorological degrees to a 16-point compass direction."""

    # Define compass directions (16-point compass)
    dirs = [
        "N", "NNE", "NE", "ENE",
        "E", "ESE", "SE", "SSE",
        "S", "SSW", "SW", "WSW",
